...
describe('storeImage', () => {
  ...
  beforeEach(() => {
    airtableRecords = new AirtableRecords('testApiKey', 'testBaseId', 'testTableId', 'testView', id)
  ...
  it('should store image and return a resolved promise', async () => {
    ...
    const mockWriteFile = jest.spyOn(fsPromises, 'writeFile').mockResolvedValue()

    await expect(airtableRecords.storeImage(imageUrl, imageFilePath)).resolves.not.toThrow()
  ...
  it('should throw error when image URL is invalid', async () => {
    ...
    await expect(airtableRecords.storeImage('invalidImageUrl', imageFilePath)).rejects.toThrowError('Invalid image URL')
  })

  it('should throw error when there is problem with file system', async () => {
    jest.spyOn(fsPromises, 'writeFile').mockImplementation(() => { throw new Error('File system error') })
    jest.spyOn(axios, 'get').mockResolvedValue({ data: Buffer.from('imageData') })
    await expect(airtableRecords.storeImage(imageUrl, imageFilePath)).rejects.toThrowError('File system error')
  })
})
